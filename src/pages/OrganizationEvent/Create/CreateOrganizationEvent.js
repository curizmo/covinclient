...
import { createOrganizationEvent } from 'services/organizationEvent';
...
const CreateOrganizationEvent = () => {
  ...
  const fetchOrganizationOptions = async (ntoUserId) => {
    try {
      ...
      const newOrganizationeEvent = {
        ...organizationEvent,
        organization: organizations[0].organizationId,
      };
  ...
  const handleSave = async () => {
    try {
      ...
      await createOrganizationEvent(organizationEvent);
  ...
  return (
    <DashboardLayout>
      <Container>
        <h2 className="mb-5">Create organization event</h2>
        <Row>
...
export { CreateOrganizationEvent };
